...
import { PUBLIC_BOOKS_PATH } from './firebase-paths';
...
export class FirebaseList {
  ...
  update(book, value) {
    return new Promise((resolve, reject) => {
      ...
      firebaseDb.ref(`${PUBLIC_BOOKS_PATH}/${book.key}`)
        .update(postData, error => error ? reject(error) : resolve());
    }
    else {
      if (firebaseDb.ref(`${PUBLIC_BOOKS_PATH}/${book.key}`) !== null) {
        firebaseDb.ref(`${PUBLIC_BOOKS_PATH}/${book.key}`)
          .remove(error => error ? reject(error) : resolve());
      }
    }
      firebaseDb.ref(`${this._path}/${book.key}`)
